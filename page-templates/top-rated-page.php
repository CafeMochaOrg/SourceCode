<?php
/**
 * Template Name: Top Rated Page Template
 *
...
<div id="primary" class="site-content">
    <div id="content" role="main">
        ...
        <div class="short_by">
            <ul>
                <li>Arrange Top Rated By-</li>
                <li><a href="<?php echo site_url() . '/top-rated/?sectionby=d' ?>">Daily</a></li>
                <li><a href="<?php echo site_url() . '/top-rated/?sectionby=w' ?>">Weekly</a></li>
                <li><a href="<?php echo site_url() . '/top-rated/?sectionby=m' ?>">Monthly</a></li>
            </ul>
        </div>
        <!--Short By-->
        <!--Slid Arrow-->
        ...
        if (is_user_logged_in()) {
            ...
            $posts_array = $wpdb->get_results("SELECT * FROM wp_posts w,wp_sipping s WHERE w.ID=s.post_id AND post_type='post' ORDER BY sip_it DESC");
        } else {
            $posts_array = $wpdb->get_results("SELECT * FROM wp_posts w,wp_sipping s WHERE w.ID=s.post_id AND post_type='post' ORDER BY sip_it DESC");
        }
        ?>
    ...
    jQuery(document).ready(function() {
        ...
        if (isset($_GET['sectionby'])) {
            global $wpdb;
            $query = "SELECT * FROM wp_posts where post_type='post' and post_status='publish' ORDER BY post_date";
            ...
            while (strtotime($last_date) <= strtotime($end_date)) {
                ...
                if (isset($_GET['sectionby'])) {
                    ...
                    $query = $query . " and post_date like '%" . date("Y-m-d", strtotime($date)) . "%'";
                //$query=$query." ORDER BY meta_value ASC";
                //echo $query."<br>";
                //$last_date=$date;
...
//                        
            ...
            if (is_user_logged_in()) {
                ...
                if ($moca == false) {
                    $moca = array();
                    $postsarray = $wpdb->get_results("SELECT * FROM `wp_posts` p LEFT JOIN wp_sipping v ON p.`ID` = v.post_id WHERE p.post_type='post' and p.post_status='publish' GROUP BY p.ID Order by SUM(v.sip_it) DESC limit 10");
                } else {
                    $mocas = implode(",", $moca);
                    $query = "SELECT * FROM wp_posts,wp_postmeta,wp_sipping WHERE wp_posts.ID=wp_postmeta.post_id AND wp_posts.ID=wp_sipping.post_id AND wp_sipping.post_id=wp_postmeta.post_id AND wp_postmeta.meta_value IN($mocas) and wp_postmeta.meta_key='mocas' and post_type='post' and post_status='publish' GROUP BY wp_posts.`ID` ORDER BY SUM(wp_sipping.sip_it) DESC limit 10";
                    //echo $query;
                    $postsarray = $wpdb->get_results($query);
                }
                //$postsarray = $wpdb->get_results("SELECT * FROM `wp_posts` p LEFT JOIN wp_sipping v ON p.`ID` = v.post_id WHERE p.post_type='post' and p.post_status='publish' Order by v.sip_it DESC");
            } else {
                $postsarray = $wpdb->get_results("SELECT * FROM `wp_posts` p LEFT JOIN wp_sipping v ON p.`ID` = v.post_id WHERE p.post_type='post' and p.post_status='publish' GROUP BY p.ID Order by SUM(v.sip_it) DESC limit 10");
            }
